...
import { useCreatePrivacyMutation, useGetPrivacyQuery } from '../features/rule/ruleApi';
...
const PrivacyPolicy = () => {
  ...
  // API hooks
  const { data: getPrivacy, isLoading: getPrivacyLoading } = useGetPrivacyQuery();
  const [createPrivacy, { isLoading: privacyLoading }] = useCreatePrivacyMutation();
  ...
  useEffect(() => {
    if (getPrivacy?.data?.content && !editorInitialized.current) {
      const content = getPrivacy.data.content.replace(/^"|"$/g, ''); // Remove quotes if present
  ...
  const handleSave = async () => {
    try {
      ...
      const result = await createPrivacy(payload).unwrap();
      ...
      message.error("An error occurred while saving privacy policy");
          ...
          >
            <div className={`py-4 sm:p-6 bg-gray-50`}>
              ...
              <Button
                ...
                disabled={privacyLoading || getPrivacyLoading}
              >
                {getPrivacyLoading ? 'Loading...' : 'Save Privacy Policy'}
              </Button>
            </div>
          </div>
        </div>
