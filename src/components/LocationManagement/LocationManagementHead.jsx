...
const LocationManagementHead = ({ columns, status, selectedInstitution }) => {
  ...
  const { data: allLocation, isLoading } = useGetAllLocationQuery({
    page: currentPage,
    institutionName: selectedInstitution === null || selectedInstitution.label === "All Institutions" ? "" : selectedInstitution?.label,  // Directly passing the ID
    status: status === null || status.label === "Select Status" ? "" : status?.value   // Directly passing the ID
  });
